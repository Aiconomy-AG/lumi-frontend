...
import {
  ...
  handleRealtimeNotificationDismissed,
...
export function RealtimeNotificationPopup() {
  ...
  useEffect(() => {
    ...
    channel.listen('.notification.dismissed', (payload: { id: number }) => {
      dismissNotification(payload.id)
      handleRealtimeNotificationDismissed(queryClient, payload.id)
    })
  ...
  if (renderedNotifications.length === 0) return null
